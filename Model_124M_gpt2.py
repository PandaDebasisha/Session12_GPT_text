...
# Add after train_loader initialization and before training loop
checkpoint_dir = 'checkpoints'
os.makedirs(checkpoint_dir, exist_ok=True)

# Function to generate sample text
def generate_sample(model, start_text="The ", max_new_tokens=30):
    model.eval()
...
for step in range(start_step, max_steps):
    ...
    if loss.item() < 0.099999:
        print(f"Loss threshold reached at step {step}. Stopping training.")
        # Save final model
        torch.save({
            'step': step,
    ...
    print(f'step {step} | loss: {loss.item():.4f} | dt: {dt:.2f}ms | tok/sec: {tokens_per_sec:.2f} | norm: {norm:.2f}')
